    ...
    if($loggedin==false) {
        if(isset($_POST["reg_user"])) {
            ...
            if($error==false) {
                ...
                $tokenurl = $config["url"]."?page=confirm&token=".$tokenurl["token"];
                mail($email,$mail["confirm_mail"],$mail["confirm_content"].$tokenurl,"From: ".$config["email"]);
                redirect("?page=signin");
            }
        }
        echo title($lang["signup"]["title"]);
?>
...
<div style="margin: 0 auto; width: 300px" id="signup_container">
    <form method="post" id="signup_form" name="reg_user">
        ...
        <div class="form-group">
            ...
            <input data-toggle="popover" data-content="Type your password again." type="password" name="password_2" id="password_2" class="form-control" placeholder="<?= $lang["signup"]["password_2"] ?>" required>
        </div>
        ...
        <?php } ?>
        <hr>
        <p class="image-shadow"><?= $lang["signup"]["signin"] ?> <a href="?page=login"><?= $lang["signin"]["title"] ?>!</a></p>
    </form>
</div>
