...
<div style="margin: 0 auto; width: 300px" id="login_container">
    <form method="post" id="login_form" name="login_user">
        ...
        <p><?= $error_msg ?></p>
        <?php } ?>
        <a href="?page=reset" class="btn btn-lg btn-warning btn-block" id="forgot_button"><?= glyph("refresh",$lang["signin"]["reset"]) ?> <?= $lang["signin"]["reset"] ?></a>
        <hr>
        <p class="image-shadow"><?= $lang["signin"]["no_account"] ?> <a href="?page=signup"><?= $lang["signup"]["title"] ?>!</a></p>
    </form>
</div>
<?php } else {
